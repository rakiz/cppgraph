...
| `SymbolInformation.signature_documentation` | stock DORMANT / **patched WORKING** | corpus (stock): 0/810,919 (0.00%); absent on fixture. Patched (patchset 5, `scip-clang-patches/signature-documentation-on-v0.4.0.patch`): emitted on defined/pure-virtual function/method declarations — a pretty-printed decl, no body, default arguments preserved; bodyless in-class declarations (no `SymbolInformation` emitted) still get none | already consumed on patched graphs: `Node.signature_documentation`, surfaced on `explain`/`explain_symbol` — signatures without a source read |
