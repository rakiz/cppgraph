...
- [`build-scip-clang-patched-linux/`](build-scip-clang-patched-linux) — compile scip-clang from source for
  the host's native arch, carrying our full patch bundle (see
  `scip-clang-patches/README.md` for the current list — PR #504
  `enclosing_range` plus our own syntactic-classifier patches). Slower to
  build, but no emulation and adds the exact-attribution feature.
