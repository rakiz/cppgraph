...
#                                             (download-patched|download|build|emulate — needed
...
list_sources() {
  ...
  local native="" native_patched="" build=0
  case "$os/$arch" in
    Darwin/arm64|Linux/x86_64) native=1 ;;
  esac
  case "$os/$arch" in
    Darwin/arm64|Linux/aarch64|Linux/arm64) native_patched=1 ;;
  esac
  case "$os" in Linux) build=1 ;; esac
  echo "platform: $os $arch"
  echo "valid scip-clang sources (pass one as --scip-source):"
  [ -n "$native_patched" ] && echo "  download-patched   prebuilt patched binary (full patch bundle — see scip-clang-patches/README.md) — ~1 min"
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --scip-source) passthrough+=("$1" "${2:?--scip-source needs download-patched|download|build|emulate}"); shift 2 ;;
